...
export default async function CampaignPage({params}: {params: Params}) {
 ...
 const questions=[
  ...
  [fr?"Que se passe-t-il si je précommande ici ?":"What happens if I pre-order here?",fr?"Vous essayez un parcours de démonstration. Le prix de 150 € et les tailles sont illustratifs. Aucun paiement, engagement ou réservation réelle n’est créé.":"You are trying a demonstration purchase journey. The €150 price and sizes are illustrative. No payment, commitment or actual reservation is created."],
 ];
 return <div className="campaign-page">
  <section className={`campaign-hero campaign-${p.slug}`}><div className="container campaign-hero-grid"><div className="campaign-copy"><p className="eyebrow">ALBEA / {p.label}</p><h1>{c.title}<br/><em>{c.accent}</em></h1><p className="hero-intro">{c.intro}</p><div className="hero-actions"><a href="#preorder" className="button">{t.join}<Icon width="18"/></a><a href="#campaign-evidence" className="text-link">{fr?"Les faits d’abord":"Start with the facts"}<Icon width="17"/></a></div><p className="campaign-micro">{fr?"2027 · France & États-Unis · Précommande démo sans paiement":"2027 · France & United States · Demo pre-order, no payment"}</p></div><div className="campaign-image"><Image src={`/images/${p.image}.webp`} alt={p.label} fill priority sizes="(max-width:760px) 100vw,50vw"/><span>{t.concept}</span></div></div></section>
  ...
  <Science/>
  <Reviews featured={p}/>
  <section className="section campaign-offer"><div className="container campaign-offer-grid"><div><p className="eyebrow">{fr?"VOTRE PROCHAIN GESTE":"YOUR NEXT STEP"}</p><h2>{fr?"Le soin de demain.":"Tomorrow’s foot care."}<br/><em>{fr?"Un aperçu aujourd’hui.":"A preview today."}</em></h2><Image src="/images/wrap-studio.webp" alt={t.imageAlt} width={900} height={600}/><p>{fr?"Découvrez la sélection de taille et le panier de précommande. Le prix, les accessoires inclus et le calendrier définitif seront confirmés au lancement.":"Explore size selection and the pre-order bag. Final pricing, included accessories and timing will be confirmed at launch."}</p></div><PurchasePanel product={product} headingLevel="h2"/></div></section>
